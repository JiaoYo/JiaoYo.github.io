import{d as E,r as c,a as u,i as j,dJ as N,b as v,o as f,e as s,f as S,g as h,t as _,aO as B,w as R,z as V,aS as L,aW as O,T as W,aI as q,aL as H}from"./index-DGUcENSr.js";import{c as K,b as U}from"./home-YUGDK0i0.js";const F={class:"map-wrapper",style:{height:"600px",position:"relative"}},J={class:"info-panel"},Y={style:{margin:"0 5px"}},Z={style:{color:"#52c41a","font-weight":"700","font-size":"24px"}},G={key:0,class:"details"},Q={style:{"margin-bottom":"10px"}},X=E({name:"trajectory_realtimetrajectory",__name:"index",setup(ee){const l=c([]),t=c(null);let r=null;async function M(){const a="/map-sdk.json",{data:e}=await q.get(`${a}?_t=${Date.now()}`);return e}function T(a){return new Promise((e,o)=>{if(window.AMap)return e(window.AMap);const n=document.createElement("script");n.src=`https://webapi.amap.com/maps?v=2.0&key=${a}`,n.defer=!0,n.onload=()=>window.AMap?e(window.AMap):o(new Error("AMap加载失败")),n.onerror=()=>o(new Error("AMap脚本加载错误")),document.head.appendChild(n)})}const p=c(!1),$=()=>{p.value=!p.value},g=u(()=>l.value.filter(a=>a.pid!=-1)),P=u(()=>l.value.filter(a=>a.pid==-1)),C=u(()=>g.value.length);u(()=>P.value.length);async function w(){const{data:a,error:e}=await K();e||(l.value=a.filter(o=>o.pid!==-1))}const d=c({contractname:"",dispatch:"",debugname:"",dbdbtime:""}),z=async a=>{const{data:e,error:o}=await U({pid:a.pid>0?a.pid:void 0,creater:a.creater});d.value=e};async function I(){const a=await M(),e=await T(a.AMAP_SDK_KEY);t.value=new e.Map("amap-container",{viewMode:"2D",zoom:5,center:[105,36]}),r=new e.InfoWindow({offset:new e.Pixel(0,-30)}),D(e),t.value.on("click",()=>{r.close()})}function D(a){console.log(l.value,"srealTimeTrack"),l.value.forEach(e=>{new a.Marker({position:[e.longitude,e.latitude],content:'<div class="marker-dot"></div>',anchor:"center",offset:new a.Pixel(0,0),zooms:[3,19]}).setMap(t.value);const n=document.createElement("div");n.className="custom-marker",n.style="display: flex;align-items: center;flex-direction: column;z-index:999;",n.innerHTML=`
    ...
    `;r.setContent(A),r.setOffset(new a.Pixel(0,-80)),r.open(t.value,i.getPosition()),t.value.setCenter(i.getPosition())}),i.on("dblclick",()=>{r.open(t.value,i.getPosition()),t.value.setCenter(i.getPosition()),t.value.setZoom(14)}),i.setMap(t.value)})}const m=c(null);return j(async()=>{await w(),m.value=window.setInterval(w,1e3*60*5),I()}),N(()=>{m.value&&clearInterval(m.value),t.value&&t.value.destroy()}),(a,e)=>(f(),v("div",F,[e[3]||(e[3]=s("div",{id:"amap-container",style:{width:"100%",height:"100%","border-radius":"8px"}},null,-1)),s("div",J,[s("div",{onClick:$,class:"cursor-pointer"},[s("span",Y,[e[0]||(e[0]=h(" 今日外勤人数：共 ")),s("span",Z,_(C.value),1),e[1]||(e[1]=h(" 人 "))]),s("span",{class:B(["arrow",{open:p.value}])},"▼",2)]),S(W,{name:"slide"},{default:R(()=>[p.value?(f(),v("div",G,[s("div",Q,[e[2]||(e[2]=s("h3",null,"外勤人员：",-1)),(f(!0),v(L,null,O(g.value,o=>(f(),v("span",{class:"fieldwork",key:o.id},_(o.username),1))),128))])])):V("",!0)]),_:1})])]))}}),ne=H(X,[["__scopeId","data-v-706e99d4"]]);export{ne as default};
