import{d as H,r as w,a as y,i as K,dP as O,b as s,o,e as a,f as u,g as p,t as l,h as N,aP as W,w as f,z as F,b1 as G,b2 as X,aT as g,aX as x,ax as Y,T as Z,aI as J,b4 as Q,aM as ee}from"./index-y_CfkRcC.js";import{c as te,b as ae}from"./home-DZTPzctL.js";import{u as ne}from"./usePageData-Bc0LzTXb.js";const se={class:"map-wrapper",style:{height:"600px",position:"relative"}},oe={class:"clockInInfo"},le={style:{color:"#1890ff","font-weight":"700","font-size":"20px"}},ie={style:{color:"#52c41a","font-weight":"700","font-size":"20px"}},re={style:{color:"#faad14","font-weight":"700","font-size":"20px"}},de={style:{color:"#8a8886","font-weight":"700","font-size":"20px"}},ce={key:0,class:"details"},pe={class:"flex-shrink-0 flex flex-col items-center justify-center w-20"},ue=["src"],fe={class:"mt-2 c-#52c41a text-sm font-bold text-center truncate w-full"},ve={class:"flex-1 flex flex-col justify-center gap-1"},me={class:"flex text-sm"},we={class:"text-[#4169e1] font-semibold"},ge={class:"flex text-sm"},xe={class:"text-[#4169e1] font-semibold"},be={class:"flex text-sm"},ye={class:"text-[#4169e1] font-semibold"},_e=H({name:"trajectory_realtimetrajectory",__name:"index",setup(ke){const v=w([]),r=w(null);let m=null;const{pageData:k,getData:S}=ne(Q);async function B(){const n="/map-sdk.json",{data:e}=await J.get(`${n}?_t=${Date.now()}`);return e}function R(n){return new Promise((e,d)=>{if(window.AMap)return e(window.AMap);const i=document.createElement("script");i.src=`https://webapi.amap.com/maps?v=2.0&key=${n}`,i.defer=!0,i.onload=()=>window.AMap?e(window.AMap):d(new Error("AMap加载失败")),i.onerror=()=>d(new Error("AMap脚本加载错误")),document.head.appendChild(i)})}const _=w(!1),h=y(()=>v.value.filter(n=>n.pid!=-1)),P=y(()=>v.value.filter(n=>n.pid==-1)),T=y(()=>k.data.filter(n=>!v.value.some(e=>e.creater==n.id))),z=w("inPeople"),U=()=>{_.value=!_.value},$=y(()=>h.value.length),C=y(()=>P.value.length);async function I(){const{data:n,error:e}=await te();e||(v.value=n)}const b=w({contractname:"",dispatch:"",debugname:"",dbdbtime:""}),V=async n=>{const{data:e,error:d}=await ae({pid:n.pid>0?n.pid:void 0,creater:n.creater});b.value=e};async function q(){const n=await B(),e=await R(n.AMAP_SDK_KEY);r.value=new e.Map("amap-container",{viewMode:"2D",zoom:5,center:[105,36]}),m=new e.InfoWindow({offset:new e.Pixel(0,-30)}),L(e),r.value.on("click",()=>{m.close()})}function L(n){console.log(v.value,"srealTimeTrack"),v.value.forEach(e=>{new n.Marker({position:[e.longitude,e.latitude],content:'<div class="marker-dot"></div>',anchor:"center",offset:new n.Pixel(0,0),zooms:[3,19]}).setMap(r.value);const i=document.createElement("div");i.className="custom-marker",i.style="display: flex;align-items: center;flex-direction: column;z-index:999;",i.innerHTML=`
    ...
    `;const c=new n.Marker({position:[e.longitude,e.latitude],content:i,anchor:"bottom-center",offset:new n.Pixel(0,-5),zooms:[3,19]});c.on("click",async()=>{var D,j,A,E;await V(e);const t=`
      <div style="
        min-width: 260px;
