import{d as Z,m as J,P as ee,Q as ae,H as w,a as y,i as te,dS as ne,b as o,o as l,e as n,f as u,g as p,t as r,h as L,aU as se,w as f,s as oe,T as le,W as re,U as b,V as x,a0 as ie,X as de,aP as ce,b4 as pe,M as ue}from"./index-K-Oe9yen.js";import{c as fe,b as me}from"./home-DQ5iTfj2.js";import{u as ve}from"./usePageData-Cn0zOp5N.js";const ge={class:"map-wrapper",style:{height:"600px",position:"relative"}},we={class:"clockInInfo"},be={style:{color:"#1890ff","font-weight":"700","font-size":"20px"}},xe={style:{color:"#52c41a","font-weight":"700","font-size":"20px"}},he={style:{color:"#faad14","font-weight":"700","font-size":"20px"}},ye={style:{color:"#8a8886","font-weight":"700","font-size":"20px"}},ke={key:0,class:"details"},_e={class:"shrink-0 flex flex-col items-center justify-center w-20"},Pe=["src"],Me={class:"mt-2 c-#52c41a text-sm font-bold text-center truncate w-full"},Te={class:"flex-1 flex flex-col justify-center gap-1"},je={class:"flex text-sm"},Se={class:"text-[#4169e1] font-semibold"},Ce={class:"flex text-sm"},Ie={class:"text-[#4169e1] font-semibold"},$e={class:"flex text-sm"},ze={class:"text-[#4169e1] font-semibold"},De=Z({name:"trajectory_realtimetrajectory",__name:"index",setup(Ae){const T=J(),j=ee(),q=ae(),m=w([]),s=w(null);let v=null;const{pageData:h,getData:R}=ve(pe),H=async()=>{await R({usertypes:"2,3"}),h.data=h.data.filter(a=>a.status==0)};async function W(){const a="/map-sdk.json",{data:e}=await ce.get(`${a}?_t=${Date.now()}`);return e}function K(a){return new Promise((e,d)=>{if(window.AMap)return e(window.AMap);const i=document.createElement("script");i.src=`https://webapi.amap.com/maps?v=2.0&key=${a}`,i.defer=!0,i.onload=()=>window.AMap?e(window.AMap):d(new Error("AMap加载失败")),i.onerror=()=>d(new Error("AMap脚本加载错误")),document.head.appendChild(i)})}const k=w(!1),_=y(()=>m.value.filter(a=>a.pid!=-1)),P=y(()=>m.value.filter(a=>a.pid==-1)),S=y(()=>h.data.filter(a=>!m.value.some(e=>e.creater==a.id))),C=w("allPeople"),O=()=>{k.value=!k.value},I=y(()=>_.value.length),$=y(()=>P.value.length);async function z(){const{data:a,error:e}=await fe();e||(m.value=a)}const g=w({contractname:"",dispatch:"",debugname:"",dbdbtime:""}),X=async a=>{const{data:e,error:d}=await me({pid:a.pid>0?a.pid:void 0,creater:a.creater});g.value=e};async function F(){const a=await W(),e=await K(a.AMAP_SDK_KEY);s.value=new e.Map("amap-container",{viewMode:"2D",zoom:5,center:[105,36]}),q.themeScheme==="dark"&&s.value.setMapStyle("amap://styles/dark"),e.plugin(["AMap.Scale","AMap.ToolBar"],()=>{s.value.addControl(new e.Scale),s.value.addControl(new e.ToolBar)}),v=new e.InfoWindow({offset:new e.Pixel(0,-30)}),G(e),s.value.on("click",()=>{v.close()})}function G(a){console.log(m.value,"srealTimeTrack"),m.value.forEach(e=>{new a.Marker({position:[e.longitude,e.latitude],content:'<div class="marker-dot"></div>',anchor:"center",offset:new a.Pixel(0,0),mapStyle:"amap://styles/dark"}).setMap(s.value);const i=document.createElement("div");i.className="custom-marker",i.style="display: flex;align-items: center;flex-direction: column;z-index:999;transform: translateX(4px);",i.innerHTML=`
    ...
    `;const c=new a.Marker({position:[e.longitude,e.latitude],content:i,anchor:"bottom-center",offset:new a.Pixel(0,-5)});c.on("click",async()=>{var D,A,E,N,B,U;await X(e);const t=`
      <div style="
        min-width: 260px;
