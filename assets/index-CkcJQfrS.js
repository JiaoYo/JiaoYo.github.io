import{d as J,z as Q,Y as ee,Z as ae,s as w,a as y,i as te,dY as ne,b as o,o as l,e as n,f as u,g as p,t as r,h as q,aL as se,w as f,G as oe,b3 as le,b4 as re,aO as b,a_ as x,a3 as ie,b2 as de,aF as ce,aS as pe,W as ue}from"./index-BTeMkgsf.js";import{d as fe,c as me}from"./home-D8WEg5c0.js";import{u as ve}from"./usePageData-BtaN_wXr.js";const ge={class:"map-wrapper",style:{height:"600px",position:"relative"}},we={class:"clockInInfo"},be={style:{color:"#1890ff","font-weight":"700","font-size":"20px"}},xe={style:{color:"#52c41a","font-weight":"700","font-size":"20px"}},he={style:{color:"#faad14","font-weight":"700","font-size":"20px"}},ye={style:{color:"#8a8886","font-weight":"700","font-size":"20px"}},ke={key:0,class:"details"},_e={class:"shrink-0 flex flex-col items-center justify-center w-20"},Pe=["src"],Me={class:"mt-2 c-#52c41a text-sm font-bold text-center truncate w-full"},Te={class:"flex-1 flex flex-col justify-center gap-1"},je={class:"flex text-sm"},Se={class:"text-[#4169e1] font-semibold"},Ce={class:"flex text-sm"},Ie={class:"text-[#4169e1] font-semibold"},$e={class:"flex text-sm"},ze={class:"text-[#4169e1] font-semibold"},De=J({name:"trajectory_realtimetrajectory",__name:"index",setup(Ae){const T=Q(),j=ee(),R=ae(),m=w([]),s=w(null);let v=null;const{pageData:h,getData:U}=ve(pe),O=async()=>{await U({usertypes:"2,3"}),h.data=h.data.filter(a=>a.status==0)};async function W(){const a="/map-sdk.json",{data:e}=await ce.get(`${a}?_t=${Date.now()}`);return e}function Y(a){return new Promise((e,d)=>{if(window.AMap)return e(window.AMap);const i=document.createElement("script");i.src=`https://webapi.amap.com/maps?v=2.0&key=${a}`,i.defer=!0,i.onload=()=>window.AMap?e(window.AMap):d(new Error("AMap加载失败")),i.onerror=()=>d(new Error("AMap脚本加载错误")),document.head.appendChild(i)})}const k=w(!1),_=y(()=>m.value.filter(a=>a.pid!=-1)),P=y(()=>m.value.filter(a=>a.pid==-1)),S=y(()=>h.data.filter(a=>!m.value.some(e=>e.creater==a.id))),C=w("allPeople"),F=()=>{k.value=!k.value},I=y(()=>_.value.length),$=y(()=>P.value.length);async function z(){const{data:a,error:e}=await fe();e||(m.value=a)}const g=w({contractname:"",dispatch:"",debugname:"",dbdbtime:""}),G=async a=>{const{data:e,error:d}=await me({pid:a.pid>0?a.pid:void 0,creater:a.creater});g.value=e};async function H(){const a=await W(),e=await Y(a.AMAP_SDK_KEY);s.value=new e.Map("amap-container",{viewMode:"2D",zoom:5,center:[105,36]}),R.themeScheme==="dark"&&s.value.setMapStyle("amap://styles/dark"),e.plugin(["AMap.Scale","AMap.ToolBar"],()=>{s.value.addControl(new e.Scale),s.value.addControl(new e.ToolBar)}),v=new e.InfoWindow({offset:new e.Pixel(0,-30)}),K(e),s.value.on("click",()=>{v.close()})}function K(a){console.log(m.value,"srealTimeTrack"),m.value.forEach(e=>{new a.Marker({position:[e.longitude,e.latitude],content:'<div class="marker-dot"></div>',anchor:"center",offset:new a.Pixel(0,0),mapStyle:"amap://styles/dark"}).setMap(s.value);const i=document.createElement("div");i.className="custom-marker",i.style="display: flex;align-items: center;flex-direction: column;z-index:999;transform: translateX(4px);",i.innerHTML=`
    ...
    `;const c=new a.Marker({position:[e.longitude,e.latitude],content:i,anchor:"bottom-center",offset:new a.Pixel(0,-5)});c.on("click",async()=>{var D,A,E,N,B,L;await G(e);const t=`
      <div style="
        min-width: 260px;
