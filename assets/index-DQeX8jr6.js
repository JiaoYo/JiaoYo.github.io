import{d as H,S as K,r as w,a as h,i as X,dR as F,b as o,o as l,e as n,f as u,g as p,t as i,h as E,aT as G,w as f,y as Y,b6 as Z,b7 as J,aX as g,a$ as x,aE as Q,W as ee,aO as ae,b8 as te,a1 as ne}from"./index-B6VmDCNW.js";import{c as se,b as oe}from"./home-BYFf6xTo.js";import{u as le}from"./usePageData-Z4O6CELG.js";const ie={class:"map-wrapper",style:{height:"600px",position:"relative"}},re={class:"clockInInfo"},de={style:{color:"#1890ff","font-weight":"700","font-size":"20px"}},ce={style:{color:"#52c41a","font-weight":"700","font-size":"20px"}},pe={style:{color:"#faad14","font-weight":"700","font-size":"20px"}},ue={style:{color:"#8a8886","font-weight":"700","font-size":"20px"}},fe={key:0,class:"details"},ve={class:"shrink-0 flex flex-col items-center justify-center w-20"},me=["src"],we={class:"mt-2 c-#52c41a text-sm font-bold text-center truncate w-full"},ge={class:"flex-1 flex flex-col justify-center gap-1"},xe={class:"flex text-sm"},be={class:"text-[#4169e1] font-semibold"},ye={class:"flex text-sm"},he={class:"text-[#4169e1] font-semibold"},ke={class:"flex text-sm"},_e={class:"text-[#4169e1] font-semibold"},Pe=H({name:"trajectory_realtimetrajectory",__name:"index",setup(Me){const B=K(),v=w([]),s=w(null);let m=null;const{pageData:b,getData:N}=le(te),R=async()=>{await N({usertypes:"2,3"}),b.data=b.data.filter(a=>a.status==0)};async function U(){const a="/map-sdk.json",{data:e}=await ae.get(`${a}?_t=${Date.now()}`);return e}function L(a){return new Promise((e,d)=>{if(window.AMap)return e(window.AMap);const r=document.createElement("script");r.src=`https://webapi.amap.com/maps?v=2.0&key=${a}`,r.defer=!0,r.onload=()=>window.AMap?e(window.AMap):d(new Error("AMap加载失败")),r.onerror=()=>d(new Error("AMap脚本加载错误")),document.head.appendChild(r)})}const k=w(!1),_=h(()=>v.value.filter(a=>a.pid!=-1)),P=h(()=>v.value.filter(a=>a.pid==-1)),T=h(()=>b.data.filter(a=>!v.value.some(e=>e.creater==a.id))),$=w("allPeople"),V=()=>{k.value=!k.value},C=h(()=>_.value.length),S=h(()=>P.value.length);async function z(){const{data:a,error:e}=await se();e||(v.value=a)}const y=w({contractname:"",dispatch:"",debugname:"",dbdbtime:""}),q=async a=>{const{data:e,error:d}=await oe({pid:a.pid>0?a.pid:void 0,creater:a.creater});y.value=e};async function O(){const a=await U(),e=await L(a.AMAP_SDK_KEY);s.value=new e.Map("amap-container",{viewMode:"2D",zoom:5,center:[105,36]}),B.themeScheme==="dark"&&s.value.setMapStyle("amap://styles/dark"),e.plugin(["AMap.Scale","AMap.ToolBar"],()=>{s.value.addControl(new e.Scale),s.value.addControl(new e.ToolBar)}),m=new e.InfoWindow({offset:new e.Pixel(0,-30)}),W(e),s.value.on("click",()=>{m.close()})}function W(a){console.log(v.value,"srealTimeTrack"),v.value.forEach(e=>{new a.Marker({position:[e.longitude,e.latitude],content:'<div class="marker-dot"></div>',anchor:"center",offset:new a.Pixel(0,0),mapStyle:"amap://styles/dark"}).setMap(s.value);const r=document.createElement("div");r.className="custom-marker",r.style="display: flex;align-items: center;flex-direction: column;z-index:999;transform: translateX(4px);",r.innerHTML=`
    ...
    `;const c=new a.Marker({position:[e.longitude,e.latitude],content:r,anchor:"bottom-center",offset:new a.Pixel(0,-5)});c.on("click",async()=>{var D,I,j,A;await q(e);const t=`
      <div style="
        min-width: 260px;
